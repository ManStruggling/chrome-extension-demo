...
chrome.runtime.onInstalled.addListener(function () {
  chrome.storage.sync.set({
    ...
    watermarkText: "extension helper"
  });

  function setClock() {
    chrome.alarms.clearAll();
    chrome.storage.sync.get(
      ["interval", "startTime", "endTime", "open", "badgeColor", "badgeText"],
      function (data) {
        const { interval, startTime, endTime, open, badgeText, badgeColor } = data;

        chrome.browserAction.setBadgeText({ text: open ? badgeText.on : badgeText.off });
        chrome.browserAction.setBadgeBackgroundColor({
          color: open ? badgeColor.on : badgeColor.off,
        });
    ...
    chrome.contextMenus.onClicked.addListener(function(info, tab) {
      switch(info.menuItemId){
          case 'baidu-search':
              ...
              break;
          case "option-config":
            chrome.tabs.create({url: "chrome-extension://picfpiennajjapdnnoejambgpnhnohgj/options.html"})
            break;
          default:;
      }
    });
